...
public class BinarySearchTree : IEnumerable<int> {
    ...
    private static void InsertMiddle(int[] sortedNumbers, int first, int last, BinarySearchTree bst) {
        ...
        // This results in the number of numbers left to be added
        int count = last - first + 1; 

        // This will result in one less than the middle of the numbers, which is okay
        // because index starts at 0 anyway. So half of 7 would be 3, which would result
        // in the number 4 at index [3]. That's the number in the middle :D
        int middleIndex = first + ((count - 1) / 2);

        // Only run if there is something in the list
        if (count > 0) { 
            bst.Insert(sortedNumbers[middleIndex]);
        }
        // Only run if there were 2 or more things left to add
        if (count > 1) { 
            InsertMiddle(sortedNumbers, middleIndex + 1, last, bst);
        }
        // Only run if there were 3 or more things left to add
        if (count > 2) {
